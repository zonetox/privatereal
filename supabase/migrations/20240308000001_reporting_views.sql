...
SELECT 
    ...
    c.risk_profile,
    COALESCE(SUM(p.capital_allocated), 0) as total_property_investment,
    COUNT(p.id) filter (where p.id is not null) as asset_count,
    COALESCE(AVG(p.expected_irr), 0) as avg_expected_roi,
    (
        SELECT stage 
...
SELECT 
    ...
    COALESCE(mi.rental_yield, 0) as avg_rental_yield,
    mi.demand_level,
