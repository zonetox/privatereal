...
const STATUS_STYLES: Record<string, { badge: string; labelKey: string }> = {
    ...
    contract_signed: {
        ...
        labelKey: 'status_contract_signed',
    },
    paid: {
...
function StatusBadge({ status, t }: { status: string | null, t: any }) {
    ...
    return (
        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-md text-xs font-semibold tracking-wide ${config.badge}`}>
            {config.labelKey ? t(`MyProperties.${config.labelKey}`) : status}
        </span>
    );
}
        ...
        <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            {properties.length === 0 && (
                <div className="flex flex-col items-center justify-center min-h-[40vh] gap-4 text-center rounded-2xl border border-slate-800 bg-slate-900/40 p-12">
                    ...
                    <p className="text-slate-300 font-semibold">{t('MyProperties.empty_title')}</p>
                    <p className="text-slate-500 text-sm max-w-xs">
                        {t('MyProperties.empty_desc')}
                    </p>
                </div>
            )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 items-start">
                    ...
                    >
                        ...
                        <div className="flex items-start justify-between gap-3">
                            ...
                            <StatusBadge status={prop.current_status} t={t} />
                        </div>

                        {/* Details Grid */}
                        <div className="grid grid-cols-2 gap-3 pt-1">
                            <div className="space-y-0.5">
                                <p className="text-[10px] uppercase tracking-widest text-slate-600 font-medium">{t('MyProperties.label_unit')}</p>
                                <p className="text-sm text-slate-200 font-semibold">{prop.unit_code ?? '—'}</p>
                            </div>
                            <div className="space-y-0.5">
                                <p className="text-[10px] uppercase tracking-widest text-slate-600 font-medium">{t('MyProperties.label_purchase_date')}</p>
                                <p className="text-sm text-slate-200 font-semibold">{formatDate(prop.purchase_date, locale)}</p>
                            </div>
                            <div className="col-span-2 space-y-0.5">
                                <p className="text-[10px] uppercase tracking-widest text-slate-600 font-medium">{t('MyProperties.label_contract_value')}</p>
                                                ...
                                                <div className="flex-1 space-y-1">
                                                    <div className="flex items-center justify-between gap-2">
                                                        <p className={`text-xs font-bold leading-none ${isCompleted ? 'text-slate-200' : 'text-slate-400'}`}>
                                                            {t(`MyProperties.milestone_${ms.milestone_type}`)}
                                                        </p>
                                                    ...
                                                    <p className="text-[10px] text-slate-500 font-medium">{formatDate(ms.milestone_date, locale)}</p>
                                    ...
                                    <details className="group">
                                        <summary className="text-xs text-slate-500 leading-relaxed font-medium line-clamp-2 cursor-pointer list-none">
                                            <span className="group-open:hidden">
                                                ...
                                                <span className="text-yellow-600/70 hover:text-yellow-500 transition-colors">{t('MyProperties.view_full_note')} →</span>
                                            </span>
                                        </summary>
