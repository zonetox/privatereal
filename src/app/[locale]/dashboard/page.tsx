    ...
    <div className="space-y-12 animate-in fade-in duration-700">
          ...
          <div className="flex items-start justify-between">
            <div className="space-y-4 flex-1">
              ...
              <div className="space-y-2">
                <span className={`inline-flex items-center px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest border ${RISK_BADGE_STYLE[client.risk_tolerance || 'balanced']}`}>
                  {client.risk_tolerance || t('DashboardOverview.risk_analyzing')}
                </span>
                <p className="text-xs text-slate-500 font-medium">{t('DashboardOverview.risk_subtitle')}</p>
              </div>
            </div>
            <div className="p-3 rounded-xl bg-slate-800/50 text-yellow-600/80 group-hover:text-yellow-500 transition-colors">
              <Shield size={20} />
      ...
      <div className="rounded-3xl border border-yellow-800/20 bg-gradient-to-br from-yellow-700/5 to-transparent p-8 flex flex-col md:flex-row items-center justify-between gap-8">
        <div className="space-y-2 text-center md:text-left">
          <h2 className="text-xl font-bold text-slate-200">{t('DashboardOverview.advisory_precision_title')}</h2>
